...
<head>
    ...
    .option-icon {
        ...
        position: fixed;
        right: 30px;
        top: 16vh; /* Changed from 11vh to 15vh */
    ...
    .dark-mode .home-option-icon {
        color: #fff;
    }

    .dark-mode .services-option-icon {
        color: #66b3ff;
    }
    ...
    .feedback-section {
        ...
        border-radius: 10px;
        padding: 20px;
        box-shadow: 3px 3px 0px rgba(0, 0, 0, 0.651);
    }
    ...
    .feedback-form input[type="email"]:invalid:not(:placeholder-shown) {
        border-color: #ff0000;
    }
    
    .feedback-form input[type="email"]:invalid:not(:placeholder-shown) + label {
        color: #ff0000;
    }
    ...
    .dark-mode .feedback-form input[type="email"]:invalid:not(:placeholder-shown) {
        border-color: #ff6666;
    }
    
    .dark-mode .feedback-form input[type="email"]:invalid:not(:placeholder-shown) + label {
        color: #ff6666;
    }
...
<body>
    ...
    <div id="home-option-icon" class="option-icon home-option-icon" style="display: none;">
        <i class="fas fa-tools"></i>
    </div>
    <div id="services-option-icon" class="option-icon services-option-icon" style="display: none;">
        <i class="fas fa-tools"></i>
    </div>
    <div id="about-option-icon" class="option-icon about-option-icon" style="display: none;">
        <i class="fas fa-tools"></i>
    </div>
    <div id="contacts-option-icon" class="option-icon contacts-option-icon" style="display: none;">
        <i class="fas fa-tools"></i>
    </div>
    <main>
        <div id="home-page">
        ...
        <div id="contacts-page" style="display: none;">
            <div class="contacts-container">
                ...
                <div class="feedback-section">
                    <h3 class="feedback-title" data-translate="feedbackSection">Feedback Section</h3>
                    <form class="feedback-form" id="feedbackForm" action="submit_feedback.php" method="POST">
                        <div class="input-container">
                            ...
                            <label for="fullName" autocomplete="off" data-translate="fullName">Enter Full Name</label>
                        </div>
                        <div class="input-container">
                            <input type="email" name="email" id="email" required placeholder=" ">
                            <label for="email" autocomplete="off" data-translate="email">Enter Email Address</label>
                        </div>
                        <div class="input-container">
                            <textarea name="comment" id="comment" required placeholder=" "></textarea>
                            <label for="comment" autocomplete="off" data-translate="commentSuggestion">Comments/Suggestions</label>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const homeOptionIcon = document.getElementById('home-option-icon');
            const servicesOptionIcon = document.getElementById('services-option-icon');
            const aboutOptionIcon = document.getElementById('about-option-icon');
            const contactsOptionIcon = document.getElementById('contacts-option-icon');
        
            function showPage(pageId) {
                pages.forEach(page => page.style.display = 'none');
                document.getElementById(pageId).style.display = 'block';
                
                // Show or hide the appropriate option icon based on the current page
                homeOptionIcon.style.display = pageId === 'home-page' ? 'block' : 'none';
                servicesOptionIcon.style.display = pageId === 'services-page' ? 'block' : 'none';
                aboutOptionIcon.style.display = pageId === 'about-page' ? 'block' : 'none';
                contactsOptionIcon.style.display = pageId === 'contacts-page' ? 'block' : 'none';
            ...
            servicesOptionIcon.addEventListener('click', function(e) {
                e.stopPropagation();
                showServiceManager();
            });
        
            aboutOptionIcon.addEventListener('click', function(e) {
                e.stopPropagation();
                showAboutEditor();
            });
        
            contactsOptionIcon.addEventListener('click', function(e) {
                e.stopPropagation();
                showContactsManager();
            });
            ...
            function updateServiceManager() {
                ...
                const manager = popup.querySelector('.service-manager');
                manager.innerHTML = '';
                
                const services = document.querySelectorAll('.service-box');
                services.forEach((service, index) => {
                    const title = service.querySelector('h3').textContent;
                    const image = service.querySelector('img').src;
                    const description = service.querySelector('.service-box-back p').textContent;
                    
                    const serviceEditor = createServiceEditor(index, title, image, description);
                    manager.appendChild(serviceEditor);
                });
            }
        
            function createServiceEditor(index, title, image, description) {
                ...
                editor.innerHTML = `
                    ...
                    <div class="input-container">
                        ...
                        <input type="text" id="service-title-${index}" value="${title}">
                    </div>
                    <div class="input-container">
                        <label for="service-image-${index}">Image URL:</label>
                        <div class="image-input-container">
                            <input type="text" id="service-image-${index}" value="${image}">
                            <label for="service-image-upload-${index}" class="upload-icon">
                                <i class="fas fa-upload"></i>
                            </label>
                            <input type="file" id="service-image-upload-${index}" accept="image/*" style="display: none;">
                        </div>
                    </div>
                    <div class="input-container">
                        <label for="service-description-${index}">Description:</label>
                        <textarea id="service-description-${index}">${description}</textarea>
                    </div>
                    <button class="remove-service-btn">Remove Service</button>
                `;
        
                // Add event listener for file upload
                const fileInput = editor.querySelector(`#service-image-upload-${index}`);
                const imageUrlInput = editor.querySelector(`#service-image-${index}`);
                ...
                const removeBtn = editor.querySelector('.remove-service-btn');
            ...
            function applyServiceChanges() {
                ...
                // Remove all existing service boxes
                services.forEach(service => service.remove());
                
                // Create new service boxes based on editors
                editors.forEach((editor, index) => {
                    const title = editor.querySelector(`#service-title-${index}`).value;
                    const image = editor.querySelector(`#service-image-${index}`).value;
                    const description = editor.querySelector(`#service-description-${index}`).value;
                    
                    const newService = createNewServiceBox(title, image, description);
                    servicesContainer.appendChild(newService);
                });
            }
        
            function createNewServiceBox(title, image, description) {
            ...
            function showAboutEditor() {
                    ...
                    <div class="about-popup-content">
                        ...
                        <div class="about-editor">
                            <div class="input-container">
                                ...
                                <input type="text" id="alert-title" value="${document.querySelector('.alert-desc h3').textContent}">
                            </div>
                            <div class="input-container">
                                <label for="alert-description">Description:</label>
                                <textarea id="alert-description">${document.querySelector('.alert-desc p').textContent}</textarea>
                            </div>
                        </div>
                        <button class="apply-btn">Apply Changes</button>
                    </div>
                ...
                const applyBtn = popup.querySelector('.apply-btn');
                applyBtn.onclick = function() {
                    const newTitle = document.getElementById('alert-title').value;
                    const newDescription = document.getElementById('alert-description').value;
                    
                    document.querySelector('.alert-desc h3').textContent = newTitle;
                    document.querySelector('.alert-desc p').textContent = newDescription;
                    
                    popup.remove();
                }
            }
        
            function showContactsManager() {
        const popup = document.createElement('div');
        popup.className = 'contacts-popup';
        popup.innerHTML = `
            <div class="contacts-popup-content">
                <span class="close-popup">&times;</span>
                <h2>Manage Contacts</h2>
                <div class="contacts-manager"></div>
                <button class="apply-btn">Apply Changes</button>
            </div>
        `;
        document.body.appendChild(popup);
        ...
        // Center the popup content
        const popupContent = popup.querySelector('.contacts-popup-content');
        popupContent.style.position = 'fixed';
        ...
        applyBtn.onclick = function() {
            ...
            function updateContactsManager() {
                const popup = document.querySelector('.contacts-popup');
                const manager = popup.querySelector('.contacts-manager');
                manager.innerHTML = '';
                
                const contactBoxes = document.querySelectorAll('.contact-box');
                contactBoxes.forEach((box, index) => {
                    const title = box.querySelector('.contact-title').textContent;
                    const icon = box.querySelector('.contact-icon').src;
                    const nearestInfo = box.querySelector('.nearest-info').innerHTML;
                    
                    const contactEditor = createContactEditor(index, title, icon, nearestInfo);
                    manager.appendChild(contactEditor);
                });
            }
        
            function createContactEditor(index, title, icon, nearestInfo) {
                const editor = document.createElement('div');
                editor.className = 'contact-editor';
                editor.innerHTML = `
                                    <h3>Contact ${index + 1}</h3>
                <div class="input-container">
                    <label for="contact-title-${index}">Title:</label>
                    <input type="text" id="contact-title-${index}" value="${title}">
                </div>
                <div class="input-container">
                    <label for="contact-icon-${index}">Icon:</label>
                    <div class="image-input-container">
                        <input type="text" id="contact-icon-${index}" value="${icon}">
                        <label for="contact-icon-upload-${index}" class="upload-icon">
                            <i class="fas fa-upload"></i>
                        </label>
                        <input type="file" id="contact-icon-upload-${index}" accept="image/*" style="display: none;">
                    </div>
                </div>
                <div class="input-container">
                    <label for="contact-info-${index}">Nearest Info:</label>
                    <textarea id="contact-info-${index}">${nearestInfo}</textarea>
                </div>
            `;

            // Add event listener for file upload
            const fileInput = editor.querySelector(`#contact-icon-upload-${index}`);
            const iconInput = editor.querySelector(`#contact-icon-${index}`);
            fileInput.addEventListener('change', function(e) {
                const file = e.target.files[0];
                if (file) {
                    const reader = new FileReader();
        ...
        function applyContactChanges() {
            const contactBoxes = document.querySelectorAll('.contact-box');
            const editors = document.querySelectorAll('.contact-editor');
            
            editors.forEach((editor, index) => {
                const title = editor.querySelector(`#contact-title-${index}`).value;
                const icon = editor.querySelector(`#contact-icon-${index}`).value;
                const nearestInfo = editor.querySelector(`#contact-info-${index}`).value;
                
                const contactBox = contactBoxes[index];
                contactBox.querySelector('.contact-title').textContent = title;
                contactBox.querySelector('.contact-icon').src = icon;
                contactBox.querySelector('.nearest-info').innerHTML = nearestInfo;
            });
        }
